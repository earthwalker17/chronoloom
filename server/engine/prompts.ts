...
6. timelineEvents：本回合值得记入浮生簿的事（至多 3 条，importance 3 仅限重大转折；npcIds 为逗号连接的人物 id 串，无则 ""）。causalEntries：因果账（至少 1 条）：什么导致了什么、打开或关闭了哪些路；effectsZh/openedZh/closedZh 为「；」连接的短句串，无则 ""。
...
11. 选择定价：每个 choice 的 extra 字段是空格分隔的键值串，可含 money=铜钱代价、stamina=体力消耗、rep=所需最低声望、anchor=该选择所系的在场人物 id（玩家可直接点击人物选定它），如 "money=200 stamina=8 anchor=he_shisan"；全免费且不挂人则为空字符串 ""。有分量的行动要在虚构内定价——宴请需钱、奔走耗体、声望解锁门路但也引人注目；每回合至少 2 个选择 extra 为空；带代价的选择须在 hintZh 里自然点出（如"得备份薄礼"）。anchor 只能用 directive.focusNpcIds 中的人物。
12. npcLines：1~3 条场景内说出口的台词，每条格式为 "npcId|台词"（如 "lvyao|弦断了，可以再续。"），台词 ≤40 字，只许给 directive.focusNpcIds 中的人物，须合于各自 personaZh 的声口，不得重复 proseZh 里已有的句子。
